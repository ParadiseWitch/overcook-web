...
export const lettuceSaladRecipe: Recipe = {
  ...
  difficulty: 1,
  baseScore: 20,
  targetFood: foodDef([ingredientDef("lettuce", ["cut"])]),
};
...
export const fishSashimiRecipe: Recipe = {
  ...
  difficulty: 1,
  baseScore: 25,
  targetFood: foodDef([ingredientDef("fish", ["cut"])]),
};
...
export const tomatoSoupRecipe: Recipe = {
  ...
  difficulty: 2,
  baseScore: 40,
  targetFood: foodDef([ingredientDef("tomato", ["cut"])], ["stir-fry"]),
};
...
export const plainCakeRecipe: Recipe = {
  ...
  baseScore: 60,
  targetFood: foodDef(
    [foodDef([ingredientDef("egg", []), ingredientDef("flour", [])], ["mix"])],
    ["pan-fry"],
  ),
};
...
export function getRecipesByDifficulty(
  ...
  return ALL_RECIPES.filter(
    (r) => r.difficulty >= minDifficulty && r.difficulty <= maxDifficulty,
  );
}
