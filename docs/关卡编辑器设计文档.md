...
- **DOM 部分**：顶部栏、工具面板、属性面板使用 HTML/CSS 实现
- **Canvas 部分**：地图编辑区域使用 Phaser 渲染

这种架构的优势：

1. DOM 适合表单、按钮等 UI 组件，开发效率高
2. Canvas 适合游戏对象渲染，与游戏主体一致
3. 两者职责分离，易于维护
...
```
┌────────────────────────────────────────────────────┐
│  关卡编辑器 (DOM: HTML 顶部栏)      [保存] [导入]  │
├──────────────┬─────────────────────────────────────┤
│              │                                     │
│  工具面板    │    地图编辑区域 (Canvas: Phaser)    │
│  (DOM: HTML) │                                     │
│              │    [17x13 网格画布]                 │
│  [基础设置]  │                                     │
│  [地板]      │    - 点击放置地板和工作站           │
...
1. **网格系统**
   - 17x13 格子的网格画布（可自定义尺寸）
   - 每格 48x48 像素
   - 显示坐标辅助线

2. **交互操作**
   - 拖拽工具面板中的地板/工作站到地图上放置
   - 点击选中地图上的对象
